...
namespace pits
{
    ...
    {
        auto tm = m[root]; auto leaf = 2 * root + 1; /* 取得原始根节点的值及左叶子节点的索引 */
    ...
    template <typename T>
    void sieve(T *p, int32_t root, int32_t n)
    {
        auto tm = *(p + root); auto leaf = 2 * root + 1;
    ...
    void heapsort(T *pl, T *pr)
    {
        ...
        {
            std::swap(*pl, *(pl + (pr - pl) - j)); sieve(pl, 0, pr - pl - j);
        }
    }

    template <typename RandomIt>
    void adjust(RandomIt it, int32_t root, int32_t n)
    {
        auto tm = *(it + root); auto leaf = 2 * root + 1;

        while (leaf < n)
        {
            if (leaf + 1 < n && *(it + leaf) < *(it + leaf + 1))
            {
                ++leaf;
            }
            if (tm < *(it + leaf))
            {
                *(it + root) = *(it + leaf); root = leaf; leaf = 2 * root + 1;
            }
            else
            {
                break;
    ...
    {
        for (int32_t i = (last - first) / 2 - 1; i >= 0; --i)
        {
            adjust(first, i, last - first);
        }
        for (int32_t j = 1; j < last - first; ++j)
        {
            std::swap(*first, *(first + (last - first) - j)); adjust(first, 0, last - first - j);
        }
    }
}
